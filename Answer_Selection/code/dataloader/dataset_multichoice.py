...
from utils import *
from .load_subtitle import merge_qa_subtitle, empty_sub, load_subtitle
...
class SimpleTextData():
    ...
    def preprocess_text(self, vocab):
        ...
        # merge qa and subtitle
        self.merge_text_data()

        print(
            'Splitting long subtitles and converting words in text data to indices, timestamps from string to float.')
        word2idx = vocab.stoi
        texts = {'test' : self.data} # only
        for text in texts.values():
            for idx, e in enumerate(text):
    ...
    def load_glove(self, glove_path):
        ...
        with open(glove_path, encoding='utf-8') as f:
            for line in tqdm(f.readlines()):
                ...
                # if len(vector) == pre_embedding_dim:
                glove[word] = vector
...
class TextData(SimpleTextData):
    def __init__(self, args, vocab=None):
        ...
        if args.blackbox_distillation:
            self.blackbox_answers_path = args.blackbox_answers_path
            if self.blackbox_answers_path is None:
                raise ValueError('blackbox_answer_path must not be None')

            blackbox_answers = load_json(self.blackbox_answers_path)

            print('Blackbox Distillation mode')
            count = 0
            for answer in blackbox_answers:
                new_qid, correct_idx = answer['new_qid'], answer['correct_idx']
                for example in self.data['train']:
                    if example['new_qid'] == new_qid:
                        #if example['correct_idx'] == correct_idx:
                        #    count += 1
                        example['correct_idx'] = correct_idx
