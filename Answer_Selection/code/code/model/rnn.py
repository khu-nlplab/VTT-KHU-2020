...
class RNNEncoder(nn.Module):
    ...
    def sort_batch(self, seq, lengths):
        sorted_lengths, perm_idx = lengths.sort(0, descending=True) # 내림차순
        reverse_indices = [0] * len(perm_idx) # 배치 길이 만큼, 원래대로 되돌리기 위해서
    ...
    def forward(self, inputs, lengths):
        ...
        # 고정된 문장의 길이를 위해 <pad> 토큰 넣는데 불필요한 계산을 줄이기 위해
